    ...
    forever {
        try {
            ...
            if (pResultImage->IsIncomplete()) {
                ...
                if (frameIndex > 0 &&
                    frameIndex %
                            (std::max(1, m_acquisitionSpecs.frameRate / 8)) ==
                        0) {
                    emit latencyAndFrameNumberUpdate(
                        duration_sum /
                            std::max(1, m_acquisitionSpecs.frameRate / 8),
                        frameIndex);
                    duration_sum = 0;
...
bool FLIRChameleon::savePreset(const QString &preset) {
    try {
        ...
        if (!IsAvailable(ptrUserSetSelector) ||
            !IsWritable(ptrUserSetSelector)) {
            qCritical() << "Unable to access User Set Selector "
        ...
        if (!IsAvailable(ptrUserSet) || !IsReadable(ptrUserSet)) {
            qCritical() << "Unable to access User Set (entry retrieval). "
                           "Aborting...";
            return false;
        }
        ...
        if (!IsAvailable(ptrUserSetSave) || !IsWritable(ptrUserSetSave)) {
            qCritical() << "Unable to save to User Set. Aborting...";
        }
...
bool FLIRChameleon::loadPreset(const QString &preset) {
    try {
        ...
        if (!IsAvailable(ptrUserSetSelector) ||
            !IsWritable(ptrUserSetSelector)) {
            qCritical()
                << "Unable to access User Set Selector (enum retrieval). "
                   "Aborting...";
            return false;
        }
        ...
        if (!IsAvailable(ptrUserSetLoad) || !IsWritable(ptrUserSetLoad)) {
            qDebug() << "Unable to load to User Set. Aborting...";
        }
...
void FLIRChameleon::settingChangedSlot(const QString &name,
    ...
    try {
        if (subMenus.last() == "FLIR Chameleon" && update)
            m_pCam->Init();
        GenApi::INodeMap &genTLNodeMap = m_pCam->GetTLDeviceNodeMap();
        GenApi::INodeMap &nodeMapTLStream = m_pCam->GetTLStreamNodeMap();
        ...
        if (subMenus.last() == "FLIR Chameleon")
            ptrNode = appLayerNodeMap.GetNode(cName);
        else if (subMenus.last() == "Stream Parameters")
            ptrNode = nodeMapTLStream.GetNode(cName);
        else if (subMenus.last() == "Transport Layer")
            ptrNode = genTLNodeMap.GetNode(cName);
        if (type == SettingsNode::String) {
            ...
            if (IsAvailable(ptrEnum) && IsWritable(ptrEnum)) {
                auto ptrEnumItem =
                    ptrEnum->GetEntryByName(val.toStdString().c_str());
...
void FLIRChameleon::startAcquisitionSlot(AcquisitionSpecs acquisitionSpecs) {
    if (!m_isStreaming) {
        try {
            ...
            GenApi::INodeMap &appLayerNodeMap = m_pCam->GetNodeMap();
            updateSettings(appLayerNodeMap,
                           m_cameraSettings->findNode(
                               "Pixel Format",
                               m_cameraSettings->getRootNode()->children()[0]));
            updateSettings(
                appLayerNodeMap,
                m_cameraSettings->findNode(
                    "Width", m_cameraSettings->getRootNode()->children()[0]));
            updateSettings(
                appLayerNodeMap,
                m_cameraSettings->findNode(
                    "Height", m_cameraSettings->getRootNode()->children()[0]));
            if (TriggerInterface::triggerInstance == nullptr) {
            ...
            connect(m_acquisitionWorker, &AcquisitionWorker::streamImage, this,
                    &FLIRChameleon::streamImage);
            connect(m_acquisitionWorker,
                    &AcquisitionWorker::latencyAndFrameNumberUpdate, this,
                    &FLIRChameleon::latencyAndFrameNumberUpdate);
            ...
            updateSettings(
                appLayerNodeMap,
                m_cameraSettings->findNode(
                    "Gain", m_cameraSettings->getRootNode()->children()[0]));
            updateSettings(
                appLayerNodeMap,
                m_cameraSettings->findNode(
                    "Width", m_cameraSettings->getRootNode()->children()[0]));
...
void FLIRChameleon::createSettings() {
    ...
    GenApi::INodeMap &genTLNodeMap = m_pCam->GetTLDeviceNodeMap();
    CategoryNode *transportLayerNode =
        new CategoryNode(m_cameraSettingsRootNode, "Transport Layer");
    ...
    CategoryNode *streamLayerNode =
        new CategoryNode(m_cameraSettingsRootNode, "Stream Parameters");
    createSettingsTreeFromCam(nodeMapTLStream.GetNode("Root"), streamLayerNode);

    m_pCam->Init();
    GenApi::INodeMap &appLayerNodeMap = m_pCam->GetNodeMap();
    CategoryNode *geniCamNode =
        new CategoryNode(m_cameraSettingsRootNode, "FLIR Chameleon");
    ...
    connect(m_cameraSettings, &SettingsObject::settingChanged, this,
            &FLIRChameleon::settingChangedSlot);
}
...
int FLIRChameleon::createSettingsTreeFromCam(GenApi::CNodePtr node,
    ...
    try {
        ...
        for (it = features.begin(); it != features.end(); ++it) {
            ...
            QString name =
                QString::fromStdString(ptrFeatureNode->GetName().c_str());
            ...
            if (ptrFeatureNode->GetPrincipalInterfaceType() ==
                ...
                result =
                    result | createSettingsTreeFromCam(ptrFeatureNode, catNode);
                ...
                QString value =
                    QString::fromStdString(ptrStringNode->GetValue().c_str());
                ...
                for (it = entries.begin(); it != entries.end(); ++it) {
                    ...
                    if (IsAvailable(ptrEnumEntryNode) &&
                        IsReadable(ptrEnumEntryNode)) {
                        QString symbolic = QString::fromStdString(
                            ptrEnumEntryNode->GetSymbolic().c_str());
                        EnumItemNode *itemNode =
                            new EnumItemNode(eNode, symbolic);
...
void FLIRChameleon::updateSettings(GenApi::INodeMap &nodeMap,
                                   SettingsNode *parent) {
    QList<QString> simpleSettings = {
        "ExposureAuto", "ExposureTime", "GainAuto", "Gain",     "Width",
        "Height",       "OffsetX",      "OffsetY",  "ReverseX", "ReverseY"};
    if (parent->type() == SettingsNode::Root ||
        parent->type() == SettingsNode::Category) {
        for (const auto &child : parent->children()) {
            updateSettings(nodeMap, child);
        }
    } else {
        try {
            GenApi::CNodePtr ptrNode =
                nodeMap.GetNode(parent->name().toStdString().c_str());
            ...
            if (ptrNode->GetPrincipalInterfaceType() == GenApi::intfIString) {
                ...
                if (simpleSettings.contains(parent->name())) {
                    emit simpleSettingChanged(
                        parent->name(),
                        QString::number(ptrFloatNode->GetValue()), writeable,
                        min, max);
                }
                if (parent->name() == "AcquisitionFrameRate") {
                    m_frameRate =
                        std::max(1, static_cast<int>(ptrFloatNode->GetValue()));
                }
            } else if (ptrNode->GetPrincipalInterfaceType() ==
                       GenApi::intfIBoolean) {
                GenApi::CBooleanPtr ptrBooleanNode =
                ...
                if (simpleSettings.contains(parent->name())) {
                    emit simpleSettingChanged(
                        parent->name(),
                        QString::number(ptrBooleanNode->GetValue()), writeable,
                        0, 0);
                }
            } else if (ptrNode->GetPrincipalInterfaceType() ==
                       GenApi::intfIEnumeration) {
                ...
                for (it = entries.begin(); it != entries.end(); ++it) {
                    ...
                    if (IsAvailable(ptrEnumEntryNode) &&
                        ...
                        QString symbolic = QString::fromStdString(
                            ptrEnumEntryNode->GetSymbolic().c_str());
                        EnumItemNode *itemNode =
                            new EnumItemNode(parent, symbolic);
                        static_cast<EnumNode *>(parent)->addItem(itemNode);
                        if (ptrCurrentEnumEntryNode == ptrEnumEntryNode)
                            static_cast<EnumNode *>(parent)->setCurrentItem(
                                itemNode);
                    }
                }
                static_cast<QComboBox *>(parent->dataField())
                    ->setCurrentText(
...
void FLIRChameleon::changeSimpleSetting(const QString &setting,
    ...
    if (setting == "ExposureAuto" || setting == "GainAuto") {
        ...
        if (val == "0")
            ...
            valSend = "Continuous";
        settingChangedSlot(setting, {"FLIR Chameleon"},
                           SettingsNode::Enumeration, valSend, true);
    } else if (setting == "ExposureTime" || setting == "Gain") {
        settingChangedSlot(setting, {"FLIR Chameleon"}, SettingsNode::Float,
                           val, true);
    } else if (setting == "Width" || setting == "Height" ||
               setting == "OffsetX" || setting == "OffsetY") {
        settingChangedSlot(setting, {"FLIR Chameleon"}, SettingsNode::Integer,
                           val, true);
    } else if (setting == "ReverseX" || setting == "ReverseY") {
        settingChangedSlot(setting, {"FLIR Chameleon"}, SettingsNode::Boolean,
                           val, true);
    }
}
...
bool FLIRChameleon::setupCamera(const CameraSettings &cameraSettings) {
    try {
        ...
        settingChangedSlot(
            "ExposureTime", {"FLIR Chameleon"}, SettingsNode::Float,
            QString::number(cameraSettings.exposureTime * 1000.0), false);
        ...
        settingChangedSlot("Gain", {"FLIR Chameleon"}, SettingsNode::Float,
                           QString::number(cameraSettings.gain), true);
        if (deinit) {
            m_pCam->DeInit();
        }
        return true;
...
void FLIRChameleon::streamImageSlot(QImage img) { emit streamImage(img); }
...
QString FLIRChameleon::getDefaultUserSet() {
    try {
        ...
        if (!IsAvailable(ptrUserSetDefault) || !IsReadable(ptrUserSetDefault)) {
            qCritical() << "Unable to access User Set Default(enum retrieval). "
                           "Aborting...";
        }
...
bool FLIRChameleon::setDefaultUserSet(const QString &userSet) {
    try {
        ...
        GenApi::CEnumEntryPtr ptrUserSet =
            ptrUserSetSelector->GetEntryByName(userSet.toStdString().c_str());
        if (!IsAvailable(ptrUserSet) || !IsReadable(ptrUserSet)) {
            qCritical() << "Unable to access User Set (entry retrieval). "
                           "Aborting...";
            return false;
...
bool FLIRChameleon::saveUserSetToFile(const QString &userSet,
    ...
    if (&m_pCam->FileSelector == NULL) {
        qCritical() << "File selector not supported on device!";
        return false;
    }
    ...
    for (unsigned int i = 0; i < selectorList.size(); i++) {
        ...
        if (!node || !IsReadable(node)) {
            qDebug() << node->GetSymbolic() << " not supported!";
            continue;
        }
        ...
        if (node->GetSymbolic().compare(userSet.toStdString().c_str()) == 0) {
            ...
            if (m_pCam->FileAccessLength.GetValue() <
                ...
                try {
                    ...
                    qCritical() << "Unable to set FileAccessLength to "
                                   "FileAccessBuffer length : "
                                << e.what();
                }
            }
            m_pCam->FileAccessOffset.SetValue(0);

            int64_t intermediateBufferSize =
                m_pCam->FileAccessLength.GetValue();
            int64_t iterations =
                (bytesToRead / intermediateBufferSize) +
...
bool FLIRChameleon::loadUserSetFromFile(const QString &userSet,
    ...
    if (&m_pCam->FileSelector == NULL) {
        qDebug() << "File selector not supported on device!";
        return false;
    }
    ...
    for (unsigned int i = 0; i < selectorList.size(); i++) {
        ...
        if (node->GetSymbolic().compare(userSet.toStdString().c_str()) == 0) {
            ...
            if (openFileToWrite() != true) {
                qDebug() << "Failed to open file!";
                if (!closeFile()) {
                    qDebug() << "Problem opening file node.";
                    return false;
                }
                if (!openFileToWrite()) {
                    qDebug() << "Problem opening file node.";
            ...
            int64_t intermediateBufferSize =
                m_pCam->FileAccessLength.GetValue();
            int64_t writeIterations =
                (totalBytesToWrite / intermediateBufferSize) +
                (totalBytesToWrite % intermediateBufferSize == 0 ? 0 : 1);

            if (totalBytesToWrite == 0) {
                qCritical()
                    << "Empty Image. No data will be written to camera.";
                return false;
            }
            ...
            for (unsigned int i = 0; i < writeIterations; i++) {
                ...
                std::unique_ptr<unsigned char> tmpBuffer(
                    new unsigned char[static_cast<unsigned int>(
                        tmpBufferSize)]);
                memcpy(tmpBuffer.get(), &pData[index],
                ...
                index = index + (intermediateBufferSize <= bytesLeftToWrite
                                     ? intermediateBufferSize
                                     : bytesLeftToWrite);
                ...
                if (!executeWriteCommand()) {
                    qDebug() << "Writing to stream failed!";
                    break;
                }
